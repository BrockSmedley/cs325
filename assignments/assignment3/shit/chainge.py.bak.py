...
def changeRec(M, A, C, i, j):
    ...
    # sum-product too big to make exact change; decrement column or row
        ...
        if (j > 0):     # decrement multiple
            j -= 1

        else:           # decrement coin, reset multiple
            i -= 1
            # reached end of table w/ no solution; start reverse alg
            if (i < 0):
                i = 0          # increment i to stay  in place
                j = len(M[i])-1 # set j to max multiple of current spot

                # find column with usable data in it
